"use client";

import Link from "next/link";
import { useState } from "react";
import { Bus, Menu, X, Phone } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Sheet, SheetContent, SheetTrigger } from "@/components/ui/sheet";

const navLinks = [
  { href: "/", label: "Home" },
  { href: "/#services", label: "Services" },
  { href: "/booking", label: "Book Now" },
  { href: "/track", label: "Track" },
  { href: "/contact", label: "Contact" },
];

export function Header() {
  const [open, setOpen] = useState(false);

  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-100 bg-white/95 backdrop-blur-sm">
      <div className="container mx-auto flex h-16 items-center justify-between px-4 lg:h-18">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2">
          <Bus className="h-7 w-7 text-orange-500" />
          <div className="flex flex-col leading-tight">
            <span className="text-lg font-bold tracking-tight text-gray-900 sm:text-xl">
              Sarthak
            </span>
            <span className="-mt-1 text-[10px] font-medium tracking-wide text-gray-500 sm:text-xs">
              Tour and Travels
            </span>
          </div>
        </Link>

        {/* Desktop Navigation */}
        <nav className="hidden items-center gap-1 lg:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-md px-3 py-2 text-sm font-medium text-gray-600 transition-colors hover:bg-orange-50 hover:text-orange-600"
            >
              {link.label}
            </Link>
          ))}
        </nav>

        {/* Desktop Right Section */}
        <div className="hidden items-center gap-4 lg:flex">
          <a
            href="tel:+917498125466"
            className="flex items-center gap-2 text-sm font-medium text-gray-600 transition-colors hover:text-orange-600"
          >
            <Phone className="h-4 w-4" />
            <span>+91 74981 25466</span>
          </a>
          <Button
            asChild
            className="rounded-full bg-orange-500 px-6 font-semibold text-white shadow-md shadow-orange-500/20 transition-all hover:bg-orange-600 hover:shadow-lg hover:shadow-orange-500/30"
          >
            <Link href="/booking">Book Now</Link>
          </Button>
        </div>

        {/* Mobile Right Section */}
        <div className="flex items-center gap-2 lg:hidden">
          {/* Phone CTA visible on mobile header */}
          <a
            href="tel:+917498125466"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-orange-50 text-orange-500 transition-colors hover:bg-orange-100"
            aria-label="Call us"
          >
            <Phone className="h-4 w-4" />
          </a>

          {/* Hamburger Menu */}
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="h-9 w-9">
                <Menu className="h-5 w-5 text-gray-700" />
                <span className="sr-only">Open menu</span>
              </Button>
            </SheetTrigger>
            <SheetContent side="right" className="w-[300px] p-0">
              <div className="flex flex-col h-full">
                {/* Sheet Header */}
                <div className="flex items-center gap-2 border-b border-gray-100 px-6 py-4">
                  <Bus className="h-6 w-6 text-orange-500" />
                  <div className="flex flex-col leading-tight">
                    <span className="text-base font-bold text-gray-900">
                      Sarthak
                    </span>
                    <span className="-mt-0.5 text-[10px] font-medium tracking-wide text-gray-500">
                      Tour and Travels
                    </span>
                  </div>
                </div>

                {/* Navigation Links */}
                <nav className="flex flex-col gap-1 px-4 py-4">
                  {navLinks.map((link) => (
                    <Link
                      key={link.href}
                      href={link.href}
                      className="rounded-lg px-3 py-2.5 text-base font-medium text-gray-700 transition-colors hover:bg-orange-50 hover:text-orange-600"
                      onClick={() => setOpen(false)}
                    >
                      {link.label}
                    </Link>
                  ))}
                </nav>

                {/* Contact Section */}
                <div className="mt-auto border-t border-gray-100 px-6 py-5">
                  <div className="flex flex-col gap-3">
                    <a
                      href="tel:+917498125466"
                      className="flex items-center gap-3 text-sm font-medium text-gray-600 transition-colors hover:text-orange-600"
                    >
                      <Phone className="h-4 w-4 text-orange-500" />
                      +91 74981 25466
                    </a>
                    <a
                      href="https://wa.me/917498125466"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-3 text-sm font-medium text-gray-600 transition-colors hover:text-green-600"
                    >
                      <svg
                        className="h-4 w-4 text-green-500"
                        viewBox="0 0 24 24"
                        fill="currentColor"
                      >
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                      </svg>
                      WhatsApp Us
                    </a>
                  </div>

                  <Button
                    asChild
                    className="mt-4 w-full rounded-full bg-orange-500 font-semibold text-white shadow-md shadow-orange-500/20 hover:bg-orange-600"
                  >
                    <Link href="/booking" onClick={() => setOpen(false)}>
                      Book Now
                    </Link>
                  </Button>
                </div>
              </div>
            </SheetContent>
          </Sheet>
        </div>
      </div>
    </header>
  );
}
